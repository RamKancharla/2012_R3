...
<?oracle-xsl-mapper <!-- SPECIFICATION OF MAP SOURCES AND TARGETS, DO NOT MODIFY. -->
  ...
  <!-- GENERATED BY ORACLE XSL MAPPER 10.1.3.3.0(build 070615.0525) AT [MON FEB 15 16:56:17 IST 2010]. -->
?>
  ...
  <xsl:template match="/">
    <local:accountPubRequest>
      ...
      <local:account>
        ...
        <xsl:variable name="count" select="count(/local:accountPubRequest/local:account/local:address)"/>
        <xsl:for-each select="/local:accountPubRequest/local:account/local:address">
          <xsl:choose>
            ...
            <xsl:when test="($count > 1.0) and (local:isPrimaryAddress = &quot;true&quot;)">
              <local:address>
                <local:city>
                  <xsl:value-of select="local:city"/>
                </local:city>
